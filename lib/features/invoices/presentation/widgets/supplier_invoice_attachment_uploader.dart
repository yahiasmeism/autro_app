...
class SupplierInvoiceAttachmentUploader extends StatelessWidget {
  const SupplierInvoiceAttachmentUploader({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<SupplierInvoiceFormBloc, SupplierInvoiceFormState>(
  ...
  Widget _buildAttachmentBox(SupplierInvoiceFormLoaded state, BuildContext context) {
    return Stack(
              ...
              child: state.pickedAttachment.fold(
                () {
                  return state.attachmentUrl.fold(
                    () => _buildPlaceholder(),
                    (url) {
                      if (state.supplierInvoiceHasImageAttachment) {
                        return _buildCachedNetworkImage(url, context);
                      } else if (state.supplierInvoiceHasPdfAttachment) {
        ...
        if (state.pickedAttachment.isSome() || state.attachmentUrl.isSome())
          Positioned(
            ...
            child: InkWell(
              ...
              onTap: () => context.read<SupplierInvoiceFormBloc>().add(ClearAttachmentEvent()),
              child: Container(
  ...
  Future<void> _pickAttachment(BuildContext context) async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      context.read<SupplierInvoiceFormBloc>().add(PickAttachmentEvent(file: pickedFile));
    }
  }
