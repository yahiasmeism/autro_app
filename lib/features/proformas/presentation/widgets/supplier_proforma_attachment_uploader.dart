...
class SupplierProformaAttachmentUploader extends StatelessWidget {
  const SupplierProformaAttachmentUploader({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<SupplierProformaFormBloc, SupplierProformaFormState>(
  ...
  Widget _buildAttachmentBox(SupplierProformaFormLoaded state, BuildContext context) {
    return Stack(
              ...
              child: state.pickedAttachment.fold(
                () {
                  return state.attachmentUrl.fold(
                    () => _buildPlaceholder(),
                    (url) {
                      if (state.supplierProformaHasImageAttachment) {
                        return _buildCachedNetworkImage(url, context);
                      } else if (state.supplierProformaHasPdfAttachment) {
        ...
        if (state.pickedAttachment.isSome() || state.attachmentUrl.isSome())
          Positioned(
            ...
            child: InkWell(
              ...
              onTap: () => context.read<SupplierProformaFormBloc>().add(ClearAttachmentEvent()),
              child: Container(
  ...
  Future<void> _pickAttachment(BuildContext context) async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      context.read<SupplierProformaFormBloc>().add(PickAttachmentEvent(file: pickedFile));
    }
  }
