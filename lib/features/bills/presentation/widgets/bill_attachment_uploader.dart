...
class BillAttachmentUploader extends StatelessWidget {
  const BillAttachmentUploader({super.key});
  ...
  Widget _buildAttachmentBox(BillFormLoaded state, BuildContext context) {
    return Stack(
              ...
              child: state.pickedAttachment.fold(
                () {
                  return state.attachmentUrl.fold(
                    () => _buildPlaceholder(),
                    (url) {
                      if (state.billHasImageAttachment) {
                        return _buildCachedNetworkImage(url, context);
                      } else if (state.billHasPdfAttachment) {
        ...
        if (state.pickedAttachment.isSome() || state.attachmentUrl.isSome())
          Positioned(
            ...
            child: InkWell(
              ...
              onTap: () => context.read<BillFormBloc>().add(RemoveAttachmentEvent()),
              child: Container(
  ...
  Future<void> _pickAttachment(BuildContext context) async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      context.read<BillFormBloc>().add(PickAttachmentEvent(file: pickedFile));
    }
  }
