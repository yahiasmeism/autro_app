...
class DashboardFilterDialog extends StatefulWidget {
  const DashboardFilterDialog._(this.filterDto);
  final DashboardFilterDto? filterDto;

  static Future<DashboardFilterDto?> show(BuildContext context, {required DashboardFilterDto? filter}) async {
    final result = await DialogUtil.showStandardDialog(
      showCloseBtnWhenNotDismissible: true,
      context: context,
      child: DashboardFilterDialog._(filter),
  ...
  @override
  State createState() => _DashboardFilterDialogState();
}
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      if (fromDate != null || toDate != null) {
                        context.read<DashboardCubit>().applyFilter(DashboardFilterDto(fromDate: fromDate, toDate: toDate));
                      } else {
